...
class MultivarUNet_mae(Multivar4dVarNet):
    ...
    def test_step(self, batch, batch_idx):
        ...
        for output_dim in range(n_output_dims):
            ...
            rec_da_batch = torch.sum((torch.cat([out]).index_select(dim=1, index=torch.Tensor([output_dim]).type(torch.int64)).cuda()[0,0,:])*
                self.rec_weight.cuda()[:self.rec_weight.cpu().numpy().shape[0]//n_output_dims],0) / torch.sum(self.rec_weight.cuda()[:self.rec_weight.cpu().numpy().shape[0]//n_output_dims],0)
    ...
    def on_test_epoch_end(self):
        ...
        for output_dim in range(n_output_dims):
            ...
            rec_da = self.test_data[output_dim]
