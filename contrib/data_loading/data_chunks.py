...
def open_var_dataset(var_path, var, var_name, domain, drop_depth, fill_nan=None, mask_path=None,sst_transfo=None):
    ...
    #var_dataset = xr.Dataset({var:xr.open_dataset(var_path)[var_name].astype('float16')})
    var_dataset = xr.Dataset({var:xr.open_dataset(var_path, chunks={"time": chunks_time_size})[var_name]})
    ...
    if fill_nan is not None:
        print("filling nan ...")
        var_dataset = var_dataset.fillna(fill_nan).compute()
...
def open_multivar_datasets(vars_info,
    ...
    full_dataset = full_dataset.chunk({"variable": full_dataset.variable.shape[0], "time": chunks_time_size, "lat": full_dataset.lat.shape[0], "lon": full_dataset.lon.shape[0]})


    """
    print("### Saving to SCRATCH...  ### \n")
    # Save to Zarr (use overwrite=True during experiments)
    zarr_path = "/SCRATCH/tpicard/dataset_prechunked.zarr"
    if not os.path.exists(zarr_path):
        full_dataset.to_zarr(zarr_path, mode="w")  # store chunked data as Zarr

    print("### Saving done ### \n")
    #  Reopen the Zarr lazily
