...
def open_var_dataset(var_path, var, var_name, domain, drop_depth, fill_nan=None, mask_path=None):
    ...
    if mask_path is not None:
        ...
        print(var_dataset[var].shape)
        #mask_list = np.repeat(mask_list, var_dataset[var].shape[0], axis=0)
        #var_dataset[var].values = xr.multiply(mask_list,var_dataset[var])
        print("putting mask ...")
        ...
        #    })
        var_dataset= var_dataset.assign({
            var:xr.apply_ufunc(mask_input, var_dataset[var], input_core_dims=[['lat', 'lon']], output_core_dims=[['lat', 'lon']], kwargs={"mask_list": mask_list}, dask="allowed", vectorize=True)
            })
