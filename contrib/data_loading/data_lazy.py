...
def open_var_dataset(var_path, var, var_name, domain, drop_depth, fill_nan=None, mask_path=None,sst_transfo=None):
    ...
    #var_dataset = xr.Dataset({var:xr.open_dataset(var_path)[var_name]}) # Charge la donnée ? 

    var_dataset= xr.open_dataset(var_path, chunks={"time": 11})[[var_name]].rename({var_name : var})
    ...
    if sst_transfo is not None: #Charge la donnée
    ...
    if fill_nan is not None: # A enlever
        print("filling nan ...")
