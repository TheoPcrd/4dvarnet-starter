...
def plot_rmse_score(list_xp_name,keys=None,add_neurost=True,depth=15):
    ...
    for xp_name in list_xp_name:
        outputdir = f'{base_outputdir}rec/{xp_name}/metric/California/'
        list_dict = list_dict+[{'type_stat':'Mean','path':outputdir+f'/Eulerian_RMS_{xp_name}.pyo','name':f'{xp_name}'}] 

    print("California result")
    if keys:
        display(DictTable_color(list_dict,keys=keys))
    else:
        display(DictTable_color(list_dict))
...
def plot_rmse_score_15m(list_xp_name,keys=None,add_neurost=True):
                ...
                if key in object:
                    ...
                    if "Correlation" in key  or "Variance" in key:
                        ...
                        if max_indices[key] == i:
                            ...
                            style = self._value_to_color(value, min_val, max_val)
                            html.append("<td style='border: 1px solid #000000; padding: 8px; {1}'>{0}</td>".format("{:.0f}".format(value), style))
                    else:
                        style = self._value_to_color(value, min_val, max_val)
