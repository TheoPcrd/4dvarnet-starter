...
year = int(sys.argv[1])
xp_name = sys.argv[2]
path_rec = f"/Odyssey/private/t22picar/multivar_drifter/outputs/saved/{xp_name}/{xp_name}"
...
def save(ds: xr.Dataset, listkey: list, file_out: str, 
    ...
    ds.attrs['title'] = xp_name
    ds.attrs['project'] = 'AI4COPSEC'
    ds.attrs['summary'] = 'Total currents reconstruction at global scale'
    ds.attrs['references'] = ''
    ds.attrs['institution'] = 'IMT Atlantic'
    ds.attrs['creator_name'] = 'Theo Picard'
...
path_file = f"{path_rec}/{year}_glob"
print(path_file)

result_filepath = f"{path_file}/test_data_dim0.nc"
res_uo = xr.open_dataset(result_filepath).sel(time=slice(start_date,end_date))
res_uo = res_uo.rename({'out': 'utot'})

result_filepath = f"{path_file}/test_data_dim1.nc"
res_vo = xr.open_dataset(result_filepath).sel(time=slice(start_date,end_date))
res_vo = res_vo.rename({'out': 'vtot'})
...
if "finescale" in path_file or "8th" in path_file: 
    ...
    mask = mask.repeat(ds_maps.sizes['time'],axis=0)
...
dossier_path_daily = f'{path_rec}/rec/'
# Créer le dossier
...
folder_out = f"{dossier_path_daily}uvtot_15m_unet_{year}.nc" #{start_date.strftime('%Y-%m-%d')}_{end_date.strftime('%Y-%m-%d')}
ds_maps.to_netcdf(folder_out, 'w', format="NETCDF4", encoding=encoding)